...
interface Props {
  representative: RepresentativeI
  isOpen: boolean
  onClose: () => void
}

export const RepresentativeCardModal = ({ representative, isOpen, onClose }: Props) => {
  const cardRef = useRef<HTMLDivElement>(null)

  const handleDownloadImage = async () => {
    if (!cardRef.current || !representative) return

    try {
      const canvas = await html2canvas(cardRef.current)
      const dataUrl = canvas.toDataURL('image/png')
      const link = document.createElement('a')
      link.download = `carnet-${representative.first_name}-${representative.first_last_name}-${representative.dni}-${representative.id}.png`
      link.href = dataUrl
      link.click()
    } catch (err) {
          ...
          <div className="p-6" style={{ marginTop: '0.3rem', marginBottom: '0.3rem' }}>
            <div className="flex gap-4">
              ...
              <div className="flex-1 space-y-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-mono font-semibold text-foreground">{representative?.dni}</p>
                </div>
              </div>
            </div>
            ...
            <div className="mt-4 pt-4 space-y-2.5 flex justify-between">
              <div>
                ...
                <p className="text-sm text-foreground leading-relaxed">{representative?.address}</p>
              </div>
              ...
              <div className="text-right">
                ...
                <p className="text-sm font-mono text-foreground">{representative?.phone_number}</p>
              </div>
            </div>
            ...
            <div className="mt-4 pt-3">
              <p className="text-xs text-muted-foreground text-center">
                ID de Registro: <span className="font-mono font-semibold">{representative?.id}</span>
              </p>
            </div>
          </div>
        </div>
