...
import { RepresentativeSchema, RepresentativeSchemaT } from '@renderer/validations'
...
interface RepresentativeModalProps {
  ...
  onClose: () => void
  mode: 'create' | 'update'
  initialData?: Partial<RepresentativeI>
  onSubmit: (data: Partial<RepresentativeI>) => Promise<void>
}
...
export const RepresentativeModal = ({
  ...
  const {
    ...
    reset,
    formState: { errors }
  } = useForm<RepresentativeSchemaT>({
    resolver: zodResolver(RepresentativeSchema)
  })

  useEffect(() => {
    if (isOpen && initialData) {
      reset(initialData as RepresentativeSchemaT)
    }
  }, [isOpen, initialData])
  ...
  const handleSendForm = async (data: RepresentativeSchemaT) => {
    setLoading(true)
    try {
      await onSubmit(data)
      onClose()
